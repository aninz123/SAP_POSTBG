...
#define SQL_SELECT_1			"SELECT COUNT(A.VAT_DATE) \"TOT_INV\", TO_CHAR(A.VAT_DATE,'DD/MM/YYYY'), " \
								"		DECODE(B.BILL_TYPE_ID, 1, 'PERIODIC', 4, 'TERMINATION', 7, 'TERMINATION') BILLTYPENAME, " \
								"		C.INVOICING_CO_ID " \
								...
								"AND B.BILL_TYPE_ID IN (1,4,7) " \
								"AND A.INVOICING_CO_NAME = C.INVOICING_CO_NAME " \
								"AND A.INVOICE_TYPE = 'IN' " \
								"GROUP BY A.VAT_DATE, DECODE(B.BILL_TYPE_ID, 1, 'PERIODIC', 4, 'TERMINATION', 7, 'TERMINATION'), C.INVOICING_CO_ID " \
								"ORDER BY C.INVOICING_CO_ID ASC, DECODE(B.BILL_TYPE_ID, 1, 'PERIODIC', 4, 'TERMINATION', 7, 'TERMINATION') ASC, A.VAT_DATE ASC"
